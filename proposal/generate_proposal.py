...
def sumario_executivo():
    ...
    flow.append(
        Paragraph(
            ...
            "vocês, em <b>cinco fases sequenciais</b>, com <b>SLA contínuo de sustentação</b> "
            "começando no mês 1, e <b>três cenários de escopo</b> (manter Shopify, híbrido, "
            "ou substituir Shopify por completo) — com investimento, riscos e ROI "
            "explicitados em cada um.",
            styles["body"],
        )
    )
    ...
    flow.append(
        kpi_row(
            [
                ("ENTREGA", "5 fases", "60 a 90 dias por fase"),
                ("EQUIPE", "5-7 pessoas", "Dedicada, em rodízio"),
    ...
    flow.append(Paragraph("Os três cenários, em uma linha cada", styles["h2"]))
    cenarios = [
        [
            Paragraph("<b>A · Shopify mantido</b>", styles["body_tight"]),
            Paragraph(
                "Frontend novo + CMS + checkout do Shopify Plus. Conservador, "
                "preserva o investimento atual, não atende totalmente o pedido de controle.",
                styles["body_tight"],
            ),
            Paragraph(fmt_brl(217_000), styles["body_tight"]),
            Paragraph(fmt_brl(14_500), styles["body_tight"]),
        ],
        [
            Paragraph("<b>B · Híbrido — recomendado</b>", styles["body_tight"]),
            Paragraph(
                "Shopify só como ERP de produto. Checkout, gateway, antifraude, "
                "dados de cliente e workflow ficam todos na nossa plataforma.",
                styles["body_tight"],
            ),
            Paragraph(fmt_brl(459_000), styles["body_tight"]),
            Paragraph(fmt_brl(17_500), styles["body_tight"]),
        ],
        [
            Paragraph("<b>C · Plataforma 100% própria</b>", styles["body_tight"]),
            Paragraph(
                "Sai do Shopify. E-commerce, fiscal (via Tiny/Bling), logística "
                "e admin completos sob controle interno. Maior controle, maior risco.",
                styles["body_tight"],
            ),
            Paragraph(fmt_brl(599_000), styles["body_tight"]),
            Paragraph(fmt_brl(22_000), styles["body_tight"]),
        ],
    ]
    ...
    flow.append(
        callout(
            "Nossa recomendação",
            "Cenário B (híbrido). Mantém o investimento do time deles em Shopify "
            "como ERP de produto e fiscal, mas devolve pra 30praum o que importa: "
            "<b>jornada do fã, dados do cliente, checkout, drops, workflow e marca</b>. "
            "Custa 24% menos que o cenário C, entrega 2 meses antes, e mantém a porta "
            "aberta pra migração total no futuro se vocês quiserem.",
        )
    )
...
def fases():
    flow = section_header("09 · FASES DE ENTREGA", "Cinco fases sequenciais, cada uma com escopo, prazo e preço.")
    ...
    flow.append(
        Paragraph(
            ...
            "Os preços abaixo são do <b>cenário B (híbrido, recomendado)</b>. Para "
            "preços dos cenários A e C, ver seção 11.",
            styles["body"],
        )
    )
    ...
    fases_data = [
        ...
        {
            "n": "FASE 2",
            "titulo": "E-commerce próprio + Checkout + Gateway",
            "prazo": "60 dias úteis",
            "preco": 115_000,
            "milestones": "40% sign · 30% milestone 1 · 30% entrega",
            "entrega": [
        ...
        {
            "n": "FASE 3",
            "titulo": "Plantão checkout + Workflow Engine",
            "prazo": "45 dias úteis",
            "preco": 85_000,
            "milestones": "50% sign · 50% entrega",
            "entrega": [
        ...
        {
            "n": "FASE 4",
            "titulo": "Segurança avançada + LGPD audit + Sustentação",
            "prazo": "45 dias úteis",
            "preco": 75_000,
            "milestones": "50% sign · 50% entrega",
            "entrega": [
...
def cenarios_completos():
    flow = section_header("11 · CENÁRIOS COMPLETOS", "A, B e C — todos detalhados, com o mesmo nível de transparência.")

    flow.append(
        Paragraph(
            "Esta seção apresenta os três cenários lado a lado, com escopo, riscos, "
            "investimento e ROI projetado de cada um. <b>Recomendamos o B</b>, mas "
            "documentamos os três pra que a decisão seja informada.",
            styles["body"],
        )
    )
    ...
    # Cenário A
    flow.append(Spacer(1, 10))
    flow.append(Paragraph("CENÁRIO A · SHOPIFY MANTIDO COMO ESTÁ", styles["h2"]))
    flow.append(
        Paragraph(
            "Frontend novo + CMS + ingressos próprios. Checkout continua no Shopify Plus. "
            "Conservador, baixo risco, mas não atende ao pedido de controle total feito por vocês.",
            styles["body_tight"],
        )
    )
    ...
    a_rows = [
        ...
        ["Fase 1", "Site + CMS + arquitetura de drop", "60 dias", fmt_brl(95_000)],
        ["Fase 2", "Workflow engine + admin de pedidos (mirror Shopify)", "45 dias", fmt_brl(55_000)],
        ["Fase 3", "Plantão checkout + segurança avançada", "45 dias", fmt_brl(67_000)],
        ["Fase 4 (opcional)", "App mobile", "60 dias", fmt_brl(89_000)],
        ["", "ENTREGA TOTAL (sem app)", "", fmt_brl(217_000)],
        ["", "SLA mensal × 24m", "", fmt_brl(14_500) + " × 24 = " + fmt_brl(348_000)],
        ["", "TOTAL 24 MESES (sem app)", "", fmt_brl(565_000)],
    ]
    flow.append(_pricing_table(a_rows))

    flow.append(Spacer(1, 14))
    flow.append(Paragraph("Riscos do cenário A", styles["h3"]))
    flow.extend(
        bullets(
            [
                "<b>Continuam pagando Shopify Plus</b> (~R$ 12 mil/mês = R$ 144 mil/ano).",
                "Taxa Shopify de 0,15% a 0,5% por transação continua aplicável.",
                "Customização de checkout limitada ao que o Shopify libera.",
                "Dados do fã ficam no Shopify, vocês não têm controle pleno.",
                "Não atende o pedido explícito feito por vocês de controle total.",
            ],
            "li_tight",
        )
    )
    ...
    flow.append(Spacer(1, 14))
    # Cenário B
    flow.append(Paragraph("CENÁRIO B · HÍBRIDO (RECOMENDADO)", styles["h2"]))
    flow.append(
        Paragraph(
            "Shopify só como ERP de produto + fiscal. Checkout, gateway, dados do fã, "
            "workflow e marca todos sob controle da 30praum. Equilíbrio entre controle e risco.",
            styles["body_tight"],
        )
    )
    b_rows = [
        ["FASE", "ESCOPO", "PRAZO", "PREÇO"],
        ["Fase 1", "Site + CMS + arquitetura de drop", "60 dias", fmt_brl(95_000)],
        ["Fase 2", "E-commerce próprio + checkout + gateway + sync Shopify", "60 dias", fmt_brl(115_000)],
        ["Fase 3", "Plantão checkout + workflow engine", "45 dias", fmt_brl(85_000)],
        ["Fase 4", "Segurança avançada + LGPD + auditoria externa", "45 dias", fmt_brl(75_000)],
        ["Fase 5", "App mobile iOS + Android", "60 dias", fmt_brl(89_000)],
        ["", "ENTREGA TOTAL (com app)", "", fmt_brl(459_000)],
        ["", "SLA mensal × 24m", "", fmt_brl(17_500) + " × 24 = " + fmt_brl(420_000)],
        ["", "TOTAL 24 MESES", "", fmt_brl(879_000)],
    ]
    ...
    flow.extend(
        bullets(
            [
                "<b>Economia direta</b>: Shopify Plus vira Shopify Basic (R$ 350/mês), poupa R$ 140 mil/ano.",
                "Time da 30praum continua usando o admin Shopify pra cadastrar produto (familiar, zero treinamento).",
                "Fiscal/NFe continua sendo problema do Shopify, fora do seu risco operacional.",
                "Dados do fã, checkout e jornada ficam no seu domínio.",
                "Migração é mínima: catálogo já tá lá, vamos só ler via API.",
                "Porta aberta pra migração futura ao cenário C, se quiserem.",
            ],
            "li_tight",
        )
    )
    ...
    # Cenário C
    flow.append(Paragraph("CENÁRIO C · PLATAFORMA 100% PRÓPRIA", styles["h2"]))
    flow.append(
        Paragraph(
            "Sai do Shopify completamente. E-commerce, fiscal, logística, admin de produto, "
            "tudo construído por nós. Controle absoluto, custo e risco substancialmente maiores.",
            styles["body_tight"],
        )
    )
    c_rows = [
        ["FASE", "ESCOPO", "PRAZO", "PREÇO"],
        ["Fase 1", "Site + CMS + arquitetura de drop", "60 dias", fmt_brl(95_000)],
        ["Fase 2", "E-commerce completo (catálogo, carrinho, checkout, gateway, antifraude)", "75 dias", fmt_brl(175_000)],
        ["Fase 3", "Admin completo (produto, estoque, NFe via Tiny/Bling, logística, migração Shopify)", "60 dias", fmt_brl(145_000)],
        ["Fase 4", "Plantão checkout + workflow + LGPD + segurança avançada", "45 dias", fmt_brl(95_000)],
        ["Fase 5", "App mobile iOS + Android", "60 dias", fmt_brl(89_000)],
        ["", "ENTREGA TOTAL", "", fmt_brl(599_000)],
        ["", "SLA mensal × 24m", "", fmt_brl(22_000) + " × 24 = " + fmt_brl(528_000)],
        ["", "TOTAL 24 MESES", "", fmt_brl(1_127_000)],
    ]
    flow.append(_pricing_table(c_rows))

    flow.append(Spacer(1, 14))
    flow.append(Paragraph("Riscos adicionais do cenário C", styles["h3"]))
    flow.extend(
        bullets(
            [
                "<b>PCI compliance</b>: mesmo com tokenização, vocês entram na cadeia de custódia do cartão.",
                "<b>Compliance fiscal</b>: erro em CFOP/CST/NCM/ICMS por estado = autuação SEFAZ, multa retroativa.",
                "<b>Backup fiscal 5 anos</b>: obrigação legal, vocês passam a ser responsáveis pela infra de retenção.",
                "<b>Suporte 24/7 a gateway + fiscal</b>: se Pix cai 3am, é problema nosso (e sobe o custo do SLA).",
                "<b>Migração de catálogo</b>: 1000+ SKUs com histórico = 60-100h de trabalho, R$ 35k a R$ 60k adicionais.",
                "<b>Período de paralelismo obrigatório</b>: 60 dias rodando Shopify + próprio juntos antes do cutover.",
                "<b>Defesa de chargeback</b>: vira operação manual de vocês (ou de um terceirizado contratado por vocês).",
            ],
            "li_tight",
        )
    )
    ...
    flow.append(
        callout(
            "Cláusulas obrigatórias se vocês fecharem o cenário C",
            "Para reduzir risco operacional, no cenário C exigimos cinco cláusulas no contrato: "
            "<b>(1)</b> Fiscal terceirizado obrigatório (Tiny ou Bling como middleware NFe). "
            "<b>(2)</b> Antifraude terceirizado obrigatório (Clearsale ou Konduto). "
            "<b>(3)</b> Gateway com PCI Level 1 obrigatório (Pagar.me ou Stone). "
            "<b>(4)</b> Migração de catálogo é fase paga separada. "
            "<b>(5)</b> Período de paralelismo de 60 dias antes do cutover. "
            "Estas cláusulas não são negociáveis e protegem ambos os lados.",
            accent=AMBER,
        )
    )
...
def cronograma():
    flow = section_header("12 · CRONOGRAMA AGREGADO", "Como as fases se distribuem no tempo.")
...
def custos_infra():
    flow = section_header("13 · CUSTOS DE INFRAESTRUTURA", "O que vocês pagam direto pra fornecedores, fora do nosso contrato.")
...
def roi_projecao():
    flow = section_header("14 · ROI E PROJEÇÃO FINANCEIRA", "O número que paga a conta.")

    flow.append(
        Paragraph(
            "Esta é a defesa do investimento em uma página. Tomamos como base o "
            "cenário B (híbrido recomendado) e as edições anuais do Plantão Festival "
            "como fonte principal de retorno mensurável.",
            styles["body"],
        )
    )
    ...
    roi_rows = [
        ...
        ["Mensalidade Shopify", fmt_brl(144_000), fmt_brl(4_200), fmt_brl(-139_800)],
        ["Taxa ST Ingressos (1 edição)", fmt_brl(480_000), fmt_brl(180_000), fmt_brl(-300_000)],
        ["Investimento na plataforma (entrega)", "—", fmt_brl(459_000), fmt_brl(459_000)],
        ["SLA mensal × 12m", "—", fmt_brl(210_000), fmt_brl(210_000)],
        ["Infraestrutura (R$ 4.5k × 12m)", fmt_brl(54_000), fmt_brl(54_000), "R$ 0"],
        ["", "TOTAL ANO 1", "", fmt_brl(229_200) + " a favor de vocês"],
    ]
    ...
    ano2_rows = [
        ...
        ["", "ECONOMIA ANO 2", "", fmt_brl(229_800)],
    ]
...
def clausulas_contratuais():
    flow = section_header("15 · CLÁUSULAS CONTRATUAIS ESSENCIAIS", "Vamos contratar como adultos.")
...
def proximos_passos():
    flow = section_header("16 · PRÓXIMOS PASSOS", "Como saímos do PDF e entramos em execução.")
...
def encerramento():
    flow = section_header("17 · ENCERRAMENTO", "Por que dar esse passo agora.")
